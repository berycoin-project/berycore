Contributing to Berycore
=======

We're working hard to make *berycore* the most powerful JavaScript library for working with berycoin. Our goal is to have *berycore* be a library that can be used by anyone interested in berycoin, and to level expertise differences with great design and documentation.
...
These are some global design goals in berycore that any change must adhere.
...
When possible, berycore objects should have standard methods on an instance prototype:
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of Berycore methods. For example, calling `JSON.stringify` on an Berycore object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
#### T3 - Require 'berycore' and Look up Classes from There

This helps to make tests more useful as examples, and more independent of where they are placed. This also helps prevent forgetting to include all submodules in the berycore object.

DO:
```javascript
var berycore = require('../');
var PublicKey = berycore.PublicKey;
```
DON'T:
```javascript
...
```sh
git remote add berycoin-project git@github.com:berycoin-project/berycore.git
git pull --rebase berycoin-project master
```
...
```
Finally go to [github.com/berycoin-project/berycore](https://github.com/berycoin-project/berycore) in your web browser and issue a new pull request.

Main contributors will review your code and possibly ask for changes before your code is pulled in to the main repository.  We'll check that all tests pass, review the coding style, and check for general code correctness. If everything is OK, we'll merge your pull request and your code will be part of berycore.

If you have any questions feel free to post them to
[github.com/berycoin-project/berycore/issues](https://github.com/berycoin-project/berycore/issues).
